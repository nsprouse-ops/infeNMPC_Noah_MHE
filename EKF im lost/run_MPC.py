...
def _mpc_loop(options):
    ...
    for i in loop_iter:
        ...
        # EKF step: predict using applied MVs, update using measured T and Cc
        u_applied = np.array(mv_row)
        T_meas  = float(measured_now.get("T",  ekf.x[4]))
        Cc_meas = float(measured_now.get("Cc", ekf.x[2]))
        ekf.step(u_applied, np.array([T_meas, Cc_meas]))
        unmeasured_xhat = ekf.get_unmeasured_xhat()

        # Feed EKF disturbance estimates back to controller — fix d_UA and d_k
        # throughout the horizon so the optimizer treats them as known
        dist_est = ekf.get_disturbance_estimates()
        print(f"  EKF disturbances [step {i+1}]: d_UA = {dist_est['d_UA']:.4f},  d_k = {dist_est['d_k']:.4f}")
        _fix_disturbances(dist_est["d_UA"], dist_est["d_k"])
        new_ss = _update_steady_state_for_disturbances(controller, options, dist_est["d_UA"], dist_est["d_k"])
