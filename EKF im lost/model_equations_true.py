...
def variables_initialize(m):
    ...
    m.Measured_index = pyo.Set(initialize=["T","Cc"])
    m.Measured_display_names = ["T\;(K)","C_c\;(mol/L)"]
    m.Unmeasured_index = pyo.Set(initialize=["Ca","Cb","Cm"])
    m.Unmeasured_display_names = ["C_a\;(mol/L)", "C_b\;(mol/L)","C_m\;(mol/L)"]
    ...
    m.UA = pyo.Param(initialize=6500, mutable=False)    # Heat transfer coefficient * area
