...
def _update_steady_state_for_disturbances(controller, options, d_UA, d_k):
    """
    Re-solve the steady-state problem with EKF disturbance estimates and
    ...
    options    : Options
    d_UA, d_k  : float               current EKF disturbance estimates
    """
    m_ss = pyo.ConcreteModel()
    m_ss = _make_steady_state_model(m_ss, options)   # initially fixes d_UA=d_k=1

    # Override disturbance values with EKF estimates
    for dname, dval in [("d_UA", d_UA), ("d_k", d_k)]:
        if hasattr(m_ss, dname):
            for t in m_ss.time:
                getattr(m_ss, dname)[t].fix(float(dval))
